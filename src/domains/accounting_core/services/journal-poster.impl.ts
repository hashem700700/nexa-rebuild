...
export class JournalPosterImpl {
  async post(tx: TransactionClient, dto: any) {
    ...
    await tx.$executeRawUnsafe(`
      ...
      VALUES 
        ('${dto.contextBundle.tenant_id}', '${journalId}', 'ASSET_INVENTORY_ACC', ${debit}, 0, 'EGP', 1, ${debit}, 0),
        ('${dto.contextBundle.tenant_id}', '${journalId}', 'EXPENSE_COGS_ACC', 0, ${credit}, 'EGP', 1, 0, ${credit})
    `);
  }
}
